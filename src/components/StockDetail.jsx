...
import {
  ...
  LinearProgress, Tooltip, Snackbar, Alert,
} from '@mui/material';
...
function StockDetail() {
 ...
 // Load stock quote
  ...
  const handleWatchlistToggle = () => {
    toggleWatchlist({ symbol, name: stock?.name ?? symbol });
    ...
    <Box sx={{ minHeight: '100vh' }}>
      ...
      <Container maxWidth="lg" sx={{ py: 3 }}>
            ...
            <Paper elevation={0} className="glass-card" sx={{
              ...
              <Box sx={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', flexWrap: 'wrap', gap: 2 }}>
                ...
                <Tooltip title={inWatchlist ? 'Remove from Watchlist' : 'Add to Watchlist'}>
                  <IconButton sx={{ color: '#5C6BC0' }} onClick={handleWatchlistToggle}>
       ...
       <Snackbar open={snackOpen} autoHideDuration={2000} onClose={() => setSnackOpen(false)}>
          <Alert severity={inWatchlist ? 'success' : 'info'} variant="filled" onClose={() => setSnackOpen(false)}>
            {inWatchlist ? `${symbol} added to watchlist` : `${symbol} removed from watchlist`}
          </Alert>
        </Snackbar>
      </Container>
    </Box>
